...
sequenceDiagram
    ...
    HTTP->>Agent: { session_id, status: "pending" }
    Agent->>HTTP: await_review(session_id)
    Note over HTTP: Blocks on watch channel
    WebView->>User: Render content
    User->>WebView: Submit decision
    WebView->>Review: tauri::invoke("submit_decision")
    Review->>HTTP: Resolve watch channel
    HTTP->>Agent: Decision response
```
...
| `http_server.rs` | axum HTTP server on `:4200`. Routes: `GET /health`, `POST /api/push`, `POST /api/heartbeat` (resets review deadline for active sessions), `POST /api/reload-plugins`, MCP Streamable HTTP on `/mcp` (GET for SSE, POST for JSON-RPC, DELETE for session teardown), and mock OAuth endpoints (`GET /.well-known/oauth-protected-resource`, `GET /.well-known/oauth-authorization-server`, `POST /oauth/register`, `GET /oauth/authorize`, `POST /oauth/token`) that implement a complete mock OAuth flow so Claude Code's HTTP transport auth handshake completes instantly without real authentication. Runs on a dedicated thread with its own tokio runtime to avoid blocking the GTK event loop. Accepts a pre-bound `std::net::TcpListener` (converted to `tokio::net::TcpListener` via `from_std`) rather than binding the port itself, ensuring the port is ready before the async runtime starts. `resolve_content_type(registry, tool_name)` maps tool names to renderer names through plugin manifest renderer maps, falling back to the raw tool name if no mapping exists. `PushResponse` implements `From<ReviewDecision>` to eliminate duplicated field-by-field construction when converting review decisions to HTTP responses. Includes 13 unit tests covering content type resolution (single-plugin, multi-plugin, fallback, no-match), mock OAuth endpoints (protected resource, authorization server metadata, register, authorize redirect, authorize missing params, token response), and `From<ReviewDecision>` conversion (full fields, minimal fields) |
...
| `review.rs` | `ReviewState` — pending review management via `tokio::sync::watch` channels. `add_pending()` creates a watch channel; `subscribe()` returns a receiver that multiple `await_review` calls can use (enabling reconnection after transport timeouts). `resolve()` or `dismiss()` sends the decision. Includes 9 unit tests covering subscribe/resolve flow, nonexistent session subscribe, multi-subscriber broadcast, remove_resolved cleanup, decision readability via borrow, async await via watch channel, dismiss behavior, and resolve/dismiss on nonexistent sessions |
| `commands.rs` | Tauri IPC commands: `get_sessions`, `submit_decision`, `dismiss_session`, `get_health`, `save_file` (native save dialog for file export), `get_renderer_registry` (returns invocable renderer metadata for frontend invocation registry), `set_native_theme` (syncs the native WKWebView/window theme with the CSS theme to prevent OS dark mode from overriding CSS text colors), plus plugin management commands (`list_plugins`, `install_plugin`, `uninstall_plugin`, `install_plugin_from_file`, `install_plugin_from_registry`, `install_plugin_from_zip`, `fetch_registry`, `start_plugin_auth`, `get_plugin_auth_header` (resolves stored/env/refreshed auth token for a plugin), `store_plugin_token`, `update_plugin` (with version guard via shared `newer_version()` that rejects downgrades/same-version updates), `get_plugin_renderers`) settings/registry commands (`get_settings`, `save_settings`, `get_registry_sources`, `add_registry_source`, `remove_registry_source`, `toggle_registry_source`), and plugin preference commands (`set_plugin_update_policy`, `get_plugin_update_policy` — both accept `State<Arc<AppState>>` and use `state.plugin_store()`, consistent with DIP pattern used by all other commands). Both `start_plugin_auth` and `get_plugin_auth_header` use `PluginRegistry::resolve_plugin_auth()` to extract auth config. Plugin install/update commands delegate to `AppState::install_or_update_from_entry()` |
| `state.rs` | `AppState` — shared state containing `Mutex<SessionStore>`, `Mutex<ReviewState>`, `Mutex<PluginRegistry>`, `Mutex<McpSessionManager>`, `Mutex<Vec<RegistryEntry>>` (cached registry), and `reqwest::Client`. Provides `reload_plugins()` for full disk reload, `notify_tools_changed()` for broadcasting `notifications/tools/list_changed` to all SSE sessions, `plugins_dir()` accessor delegating to `PluginStore::dir()`, `install_plugin_from_manifest(manifest)` for upsert-style plugin registration (removes existing plugin with same name, then adds), `install_or_update_from_entry(entry)` for installing or updating a plugin from a registry entry (downloads ZIP if `entry.download_url` or `manifest.download_url` is present, otherwise manifest-only), and `plugin_csp_origins()` which returns deduplicated origins (scheme + authority) from all installed plugin MCP URLs for dynamic CSP injection. `new_with_store(store)` constructor accepts a custom `PluginStore` for testable construction with temp directories (avoids touching the real filesystem). All plugin install/uninstall/update commands now call `notify_tools_changed()` so connected MCP clients are notified immediately |
| `mcp_prompts.rs` | MCP prompts protocol implementation. `ONBOARDING_PROMPT` constant with guided plugin discovery instructions. `builtin_prompt_definitions()` returns 4-tuples `(name, description, arguments, content)` pairing prompt content with definitions for extensibility (adding a new built-in prompt requires only adding a tuple entry). `list_prompts(state)` returns all available prompts (built-in + plugin, with plugin prompts namespaced as `{plugin}/{prompt}`). `resolve_builtin_prompt(name)` looks up a built-in prompt by name from the definitions list, returning its content if found. `get_prompt(name, arguments, state)` resolves a prompt by name (built-in via `resolve_builtin_prompt`, or plugin via `{plugin}/{prompt}` split) and returns MCP-formatted messages. `call_get_plugin_prompt(arguments, state)` fetches a plugin prompt file from disk with `{{arg}}` template substitution. Includes 4 unit tests for `builtin_prompt_definitions`, `resolve_builtin_prompt` (found/not found), and definition structure validation |
...
| `renderer_scanner.rs` | Scans installed plugin directories for custom renderer JS files in `{plugin_dir}/renderers/*.js`, returning `RendererInfo` structs with `plugin://` protocol URLs. Appends `?v={mtime}` cache-busting query parameter (file modification timestamp in Unix seconds) to each renderer URL so reinstalling a plugin reliably loads new JS without requiring an uninstall/reinstall cycle. Each `RendererInfo` now includes `mcp_url: Option<String>` read from the plugin's `manifest.json` (`mcp.url` field), used by the frontend to populate `window.__mcpviews_plugins` so renderers can resolve their backend API URL without per-plugin globals |
| `registry.rs` | Re-exports `get_configured_registry_url` and `fetch_registry` from the shared crate |
| `plugin.rs` | `PluginRegistry` — plugin lifecycle management (load, add, remove, refresh). `PluginToolResult` struct for named-field plugin tool lookup results. `find_plugin_by_name(name) -> Option<(usize, &PluginManifest)>` for named plugin lookup. `resolve_plugin_auth(&self, plugin_name) -> Result<PluginAuth, String>` extracts the `PluginAuth` config for a plugin by name (finds manifest, extracts `mcp.auth`), replacing 3 previously duplicated auth-lookup blocks across `commands.rs` and `mcp_tools.rs`. `try_refresh_oauth(oauth_info, client) -> Option<String>` helper for deduplicated OAuth token refresh with logging, used by both `lookup_plugin_tool` and `refresh_stale_plugins`. `OAuthRefreshInfo` struct for refresh parameters. `fetch_plugin_tools` now captures and forwards the `mcp-session-id` response header from the initialize handshake to the subsequent initialized notification and tools/list requests, fixing session affinity with remote MCP servers. Logs a warning when tools/list returns 0 tools (previously silent). Includes 3 unit tests for `resolve_plugin_auth` covering not-found, no-auth-config, and success cases |
...
The WebView loads `index.html` which includes a `<meta name="color-scheme">` tag and an inline `syncNativeTheme()` function that calls the `set_native_theme` Tauri command on initial load and on every theme toggle, ensuring the native WKWebView rendering context matches the CSS theme. The header is split into `#header-left` (refresh button + title) and `#header-actions` (connection status indicator + theme toggle + apps dropdown). The refresh button is hidden when no session is active and shown when content is rendered:
- CDN scripts: `marked.js` (markdown), `mermaid` (diagrams)
- `styles.css` — glassmorphism design system with 140+ CSS custom properties, light/dark theme support via `[data-theme]` attribute and `color-scheme` property, frosted glass panels, geometric dot-grid background, and refined typography hierarchy. Theme toggle with `prefers-color-scheme` system preference detection. All renderer styling uses CSS classes (no inline JS styles); renderer JS files reference CSS variables exclusively (no hardcoded hex colors). Includes component-specific variables for data governance grids, badge colors, and diff card states. `#main-header` uses `z-index: 100` to establish a stacking context above renderer content (prevents backdrop-filter from trapping dropdowns). `.header-icon-btn` provides a reusable icon button style for header controls (used by the refresh button). Apps dropdown uses `z-index: 9999` to layer above all other content. Dark mode overrides for mermaid SVG elements (text fills, node/edge strokes, actor boxes, sequence diagram elements) applied via `[data-theme="dark"] .mermaid-rendered` and `.mermaid-modal-body` selectors
- `main.js` — app bootstrap, Tauri IPC event listener, multi-session tab bar with cached content containers, review countdown timers, theme toggle logic with system preference detection and `localStorage` persistence. Error states use CSS variable references (`--color-error`, `--text-secondary`) instead of hardcoded colors. Includes global `mcpview://` invocation click handler (delegated via `data-invoke-renderer` attributes), Escape key handler for closing topmost drawer, invocation registry population on startup and plugin reload, and drawer cleanup on session removal. `refreshCurrentSession()` clears the cached content container for the active session and re-renders it, triggered by the header refresh button. During plugin renderer loading, injects `window.__mcpviews_plugins` config object (keyed by plugin name, containing `mcp_url`) before loading renderer scripts, so renderers can resolve their backend API URL via the standard `__mcpviews_plugins` contract
...
- `PromptDef` — structured prompt definition with `name`, `description`, `arguments: Vec<PromptArgument>`, and `source` (relative path to prompt content file within the plugin directory). Used by `PluginManifest::prompt_definitions` for declaring plugin prompts discoverable via the MCP prompts protocol
- `PromptArgument` — prompt argument definition with `name`, `description`, and `required` (defaults to false). Used by `PromptDef` to declare template arguments for plugin prompts
- `PluginManifest`, `PluginMcpConfig` — plugin definition and MCP connection config. Manifests now support `renderer_definitions: Vec<RendererDef>` for structured renderer metadata, `tool_rules: HashMap<String, String>` for per-tool behavioral rules, `plugin_rules: Vec<String>` for high-level behavioral rules visible every session via `init_session`/`mcpviews_setup`/`get_plugin_docs`, `registry_index: Option<PluginRegistryIndex>` for a pre-authored compact index used by `init_session`, `prompt_definitions: Vec<PromptDef>` for guided workflow prompts discoverable via the MCP prompts protocol, and `download_url: Option<String>` for ZIP package location (used by `manifest_url`-based registry entries and the `update_plugins` tool). `no_auto_push: Vec<String>` is retained for backward compatibility but has no effect since auto-push was removed
- `PluginPreferences` — per-plugin preference storage with `update_policy` (`"always"`, `"ask"`, `"skip"`), `update_policy_version` (for skip-specific-version), and `update_policy_source` (`"chat"` or `"ui"`). Includes `Default` impl returning `"ask"` policy. Stored at `~/.mcpviews/plugins/{name}/preferences.json` via `PluginStore::load_preferences()` and `save_preferences()`
- `PluginRegistryIndex` — compact plugin summary for the `init_session` plugin registry: `summary`, `tags`, `tool_groups: Vec<ToolGroupEntry>`, and `renderer_names`. If not provided in the manifest, MCPViews auto-derives it from the `renderers` map and tool cache
- `ToolGroupEntry` — a named tool group within a plugin registry index: `name` (title-cased), `hint` (truncated tool description), and `tools` (list of unprefixed tool names)
...
- `RegistryEntry`, `RemoteRegistry` — remote registry schema. `RegistryEntry` includes optional `download_url` for ZIP package distribution and optional `manifest_url` for remote manifest resolution (enables providers to manage versions from their own repos without PRs to the MCPViews registry)
...
- `newer_version(installed, available) -> Option<String>` — shared semver comparison helper that returns the `available` version string if it is strictly greater than `installed`, or `None` if equal, older, or either fails to parse. Used by `commands.rs` (version guard in `update_plugin`), `mcp_tools.rs` (`collect_plugin_updates`), and `plugin.rs` (`PluginRegistry::load_all` update detection)
- Path helpers: `plugins_dir()`, `config_path()`, `auth_dir()`, `cache_dir()` — all under `~/.mcpviews/`
- `plugin_store::PluginStore` — filesystem-based plugin CRUD (list, load, save, remove, exists). `read_prompt_source(plugin_name, source_path)` reads a prompt content file from a plugin's directory, used by `mcp_prompts::call_get_plugin_prompt` for template-based prompt resolution. `load_preferences(name)` and `save_preferences(name, prefs)` manage per-plugin `PluginPreferences` stored at `{plugin_dir}/{name}/preferences.json`. Used by both CLI and Tauri app, eliminating duplicated disk I/O logic. Injected into `PluginRegistry` at construction time for testability. `dir()` accessor exposes the plugin directory path (used by `AppState::reload_plugins()` to reconstruct a fresh store)
...
- `registry` module — `get_configured_registry_url()`, `fetch_registry()`, `get_registry_sources()`, `save_registry_sources()`, and `fetch_all_registries()` with per-source 1-hour disk caching. `resolve_manifest_urls(client, entries)` fetches remote `manifest.json` for each entry with a `manifest_url`, updating the entry's manifest, version, and download URL (failures are logged and the entry is left unchanged). Supports multi-source registry configuration with fallback to legacy single `registry_url`. When all remote sources fail, falls back to a bundled registry (`bundled_registry.json`) compiled into the binary via `include_str!`. Shared by both CLI and Tauri app
...
sequenceDiagram
    ...
    Agent->>HTTP: await_review(session_id)
    Note over HTTP: HTTP handler blocks on watch channel
    WebView->>User: Render review UI
    User->>WebView: Click accept/reject
    WebView->>Review: invoke submit_decision
    Review->>HTTP: Resolve watch channel with decision
    HTTP->>Agent: Decision response

    alt Transport timeout (connection drops)
        Agent->>HTTP: await_review(session_id) [reconnect]
        Note over HTTP: Deadline resets, blocks again
    end
```
...
1. **`push_review` / `store_push`**: Creates a `tokio::sync::watch` channel, stores the session, emits to the WebView, and returns immediately with `{ session_id, status: "pending" }`
2. **`await_review` / `await_decision`**: Subscribes to the watch channel and blocks until the user submits a decision or the deadline expires. The deadline resets to the full timeout on each `await_review` call, so agents can reconnect after a transport timeout without losing the review session
3. When the user clicks accept/reject in the WebView, the `submit_decision` IPC command resolves the watch channel
4. **`push_check`**: Non-blocking status poll -- returns `"pending"` or `"decided"` without waiting
...
- **`POST /mcp`** — JSON-RPC request/response for client-to-server calls. Optional `mcp-session-id` header to bind to an existing session (returns 404 if session not found). The `initialize` response advertises both `tools` and `prompts` capabilities (with `listChanged: true`), and includes dynamic `instructions` text built from available renderer definitions and plugin auth status warnings. Supports `prompts/list` and `prompts/get` methods for native prompt discovery by MCP clients.
...
- Reviews use a non-blocking two-step flow: `push_review` returns immediately with a `session_id`; the agent calls `await_review(session_id)` to block until the user's decision. If the transport times out, the agent calls `await_review` again -- the session persists and the deadline resets
